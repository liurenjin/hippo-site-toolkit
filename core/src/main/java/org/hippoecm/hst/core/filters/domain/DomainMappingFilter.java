...
import org.hippoecm.hst.core.timers.GeneralTimer;
    ...
    public void handleRequestForThisFilter(HttpServletRequest req, ServletResponse response, FilterChain chain,
        ...
        if(!this.domainMapping.isInitialized()) {
            try {
                ...
                domainMapping.init();
                GeneralTimer.log.debug("Initializing domain mapping took: " + (System.currentTimeMillis() - start) + " ms.");
                DomainMappingTimer.log.debug("Initializing domain mapping took: " + (System.currentTimeMillis() - start) + " ms.");
        ...
        GeneralTimer.log.debug("Matching domain took: " + (System.currentTimeMillis() - start) + " ms for '{}'", req.getServerName());
        ...
        if(matchingDomain == null) {
            ...
            if(matchingDomain.isRedirect()) {
                ...
                try {
                    ...
                    session = JcrSessionPoolManager.getInstance().getSession(req, hstRequestContext, repositoryMapping.getPath());
                    ...
                    hstRequestContext.setOriginalRequestUri(req.getRequestURI());
