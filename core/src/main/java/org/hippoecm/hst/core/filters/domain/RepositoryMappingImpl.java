...
public class RepositoryMappingImpl implements RepositoryMapping {
    ...
    public RepositoryMappingImpl(Session session, DomainMapping domainMapping, Domain domain, String prefix, String path, boolean isFromTemplate) throws RepositoryMappingException{
        this.prefix = prefix;
        this.path = path;
        ...
        if(prefix.contains(PATH_WILDCARD) || path.contains(PATH_WILDCARD)) {
            ...
            this.template = false;
            init(session, isFromTemplate);
        }
        
        
    }
    
    private void init(Session session, boolean isFromTemplate) throws RepositoryMappingException {
        long start = System.currentTimeMillis();
        try {
            if(domainMapping.isSimpleMapping() && !isFromTemplate) {
                if(this.getPath().startsWith("/preview")) {
                    this.canonicalContentPath = "/content"+this.getPath().substring("/preview".length());
                    return;
                } else if(this.getPath().startsWith("/live")) {
                    this.canonicalContentPath = "/content"+this.getPath().substring("/live".length());
                    return;
                } 
            }
        ...
        DomainMappingTimer.log.debug("RepositoryMapping init took --({})-- ms.", System.currentTimeMillis() -start);
    }
