...
public class DomainMappingImpl implements DomainMapping{
    ...
    private static final String SIMPLE_MAPPING_PARAM = "simplemapping";
    ...
    public DomainMappingImpl(String domainMappingLocation, FilterConfig filterConfig){
        ...
        String simpleMappingStr = filterConfig.getInitParameter(SIMPLE_MAPPING_PARAM);
        if(simpleMappingStr != null) {
            isSimpleMapping = Boolean.parseBoolean(simpleMappingStr);
        }
    }
    ...
    public void init() throws DomainMappingException{
        ...
        try {
            ...
            try {
                ...
                DomainMappingTimer.log.debug("Creating all domain patterns took --({})-- ms.", System.currentTimeMillis()-start);
                start = System.currentTimeMillis();
                orderedDomains = domains.toArray(new Domain[domains.size()]);
                Arrays.sort(orderedDomains);
                DomainMappingTimer.log.debug("Ordering domains took --({})-- ms.", (System.currentTimeMillis() - start));
    ...
    private void createDomainPatterns(Node domainMappingNode, Set<Domain> domains, String domainPattern, int depth, ArrayList<String> repositorypaths) throws RepositoryException {
        ...
        while(mappings.hasNext()) {
            ...
            if(mappingNode != null) {
                ...
                DomainMappingTimer.log.debug("Creating domain took --({})-- ms for ({})", (System.currentTimeMillis() - start) , mappingNode.getPath());
            }
        }
    }
    ...
    public RepositoryMapping getRepositoryMapping(String repositoryPath, RepositoryMapping currentRepositoryMapping) {
        ...
        String cacheKey = currentDomain.hashCode() + "_" + currentRepositoryMapping.getPath() + "_"+ repositoryPath;
