...
import org.hippoecm.hst.core.timers.DomainMappingTimer;
...
public class DomainImpl implements Domain {
    ...
    private RepositoryMapping[] createRepositoryMappings(Session session, String[] repositoryPaths) throws DomainException{
        ...
        for(String repositoryPath : repositoryPaths) {
            ...
            try {
                if(index > -1) {
                    repoMappings.add(new RepositoryMappingImpl(session, domainMapping,this, repositoryPath.substring(0,index), repositoryPath.substring(index+1, repositoryPath.length()), false));
                } else {
                    repoMappings.add(new RepositoryMappingImpl(session, domainMapping, this, "",repositoryPath, false));
                }
            } catch (RepositoryMappingException e) {
               log.warn("Skipping Repository Mapping for pattern '{}' because {}", pattern, e.getMessage());
            }
            DomainMappingTimer.log.debug("Creating RepositoryMappingImpl took --({})-- ms.", (System.currentTimeMillis() -start));
        }
        ...
        DomainMappingTimer.log.debug("Creating all repositorymappings for domain took --({})-- ms. ", (System.currentTimeMillis() - globalStart));
        ...
        DomainMappingTimer.log.debug("Sorting repositorymappings for domain took --({})-- ms. ", (System.currentTimeMillis() - globalStart));
        ...
        for(RepositoryMapping repositoryMapping : repositoryMappings) {
            if(repositoryMapping.match(ctxStrippedUri)) {
                ...
                if(repositoryMapping.isTemplate()) {
                    ...
                    try {
                        repositoryMappingFromTemplate = new RepositoryMappingImpl(session, this.domainMapping, this, prefix, path, true);
