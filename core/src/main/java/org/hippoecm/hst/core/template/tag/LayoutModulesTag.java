...
public class LayoutModulesTag  extends BodyTagSupport {
	...
	public int doStartTag() throws JspException { 
		...
		GeneralTimer.log.debug(" <------ START CONTAINER: ({}) ------> ", name);
        ...
        for (int index=0; index < pcmList.size(); index++) {
            ...
            try {
				...
				if(debugView) {
                    ...
                    GeneralTimer.log.debug(" <--- START TEMPLATE ({}) ---> ", templatePage);
                    long templateStart = System.currentTimeMillis();
                    pageContext.include(templatePage);
                    GeneralTimer.log.debug(" <--- END TEMPLATE ({}) took --({})-- ms ---> ", templatePage, (System.currentTimeMillis() - templateStart));
                }
        ...
        GeneralTimer.log.debug(" <------ END CONTAINER: ({}) TOOK --({})-- ms.------> ", name, (System.currentTimeMillis() - start));
