...
public class HstStartRequestFilter implements Filter{
    ...
    private static final String CACHEEXPIRESMINUTES_FILTER_INIT_PARAM = "cacheExpiresMinutes";
    
    private List<String> ignorePathsList = null;
    private List<String> ignoreTypesList = null;
    private int cacheExpiresMinutes = 1;
    ...
    public void init(FilterConfig filterConfig) throws ServletException {
        ...
        String cacheExpiresMinutesStr = filterConfig.getInitParameter(CACHEEXPIRESMINUTES_FILTER_INIT_PARAM);
        if(cacheExpiresMinutesStr != null) {
            try {
                this.cacheExpiresMinutes = Integer.parseInt(cacheExpiresMinutesStr);
            } catch(NumberFormatException e) {
                log.error("Ïgnoring cacheExpiresMinutes init param because cannot be parsed to an integer.");
            }
        }
    }
    ...
    public void doFilter(ServletRequest req, ServletResponse response, FilterChain chain) throws IOException,
        ...
        if (ignoreRequest(request, servletCtxStrippedRequestUri)) {
            ...
            if(URI.startsWith("/preview")) {
                req.setAttribute(HstRequestContext.class.getName(), new HstRequestContext(true, cacheExpiresMinutes));
            } else {
                req.setAttribute(HstRequestContext.class.getName(), new HstRequestContext(false, cacheExpiresMinutes));
            }
        }
