...
public class HstFilter implements Filter {
    ...
    /**
     * Finds a resolved mount of the correct type ('preview' or 'live') for a host in a host group in the given virtual
     * host. We take as Mount the mount that has the closest content path {@link Mount#getCanonicalContentPath()} to the
     * <code>nodePath</code>. If multiple {@link Mount}'s have an equally well suited {@link
     * Mount#getCanonicalContentPath()}, we pick the mount  with the fewest types is picked. These mounts are in general
     * the most generic ones. If multiple {@link Mount}'s have equally well suited {@link
     * Mount#getCanonicalContentPath()} and equal number of types, we pick one at random
     *
     ...
     * @throws org.hippoecm.hst.configuration.hosting.MatchException
     *          when no matching mount could be found for the given host and type.
     */
    ...
    /**
     * Sets the HST equivalent of the servletPath on the container request, namely the resolved path of the resolved HST
     * mount.
     *
    ...
    private void sendRedirectToUuidUrl(HttpServletRequest req, HttpServletResponse res, HstMutableRequestContext requestContext,
        ...
        try {
            ...
            setHstServletPath((GenericHttpServletRequestWrapper) containerRequest, mount);
    ...
    /**
     * Cleaning up resources when the entire hst request processing got skipped but there was already a jcr session
     * taken from the session pool. This currently can happen when some {@link HstSiteMapItemHandler} impl calls {@link
     * HstRequestContext#getSession} and the returns <code>null</code> from its  {@link
     * HstSiteMapItemHandler#process(org.hippoecm.hst.core.request.ResolvedSiteMapItem,
     * javax.servlet.http.HttpServletRequest, javax.servlet.http.HttpServletResponse)} method, short circuiting the hst
     * request handling (thus, during request matching, not even invoking a single {@link
    ...
    /**
     * This method is invoked for every {@link HstSiteMapItemHandler} from the resolvedSiteMapItem that was matched from
     * {@link ResolvedMount#matchSiteMapItem(String)}. If in the for loop the <code>orginalResolvedSiteMapItem</code>
     * switches to a different newResolvedSiteMapItem, then still the handlers for
     * <code>orginalResolvedSiteMapItem</code> are processed and not the one from <code>newResolvedSiteMapItem</code>.
     * If some intermediate {@link HstSiteMapItemHandler#process(ResolvedSiteMapItem, HttpServletRequest,
     * HttpServletResponse)} returns <code>null</code>, the loop and processing is stooped, and <code>null</code> is
     * returned. Entire request processing at that point is assumed to be completed already by one of the {@link
     * HstSiteMapItemHandler}s (for example if one of the handlers is a caching handler). When <code>null</code> is
     * returned, request processing is stopped.
     *
     * @param orginalResolvedSiteMapItem
     * @param siteMapItemHandlerFactory
     * @param req
     * @param res
     * @return a new or original {@link ResolvedSiteMapItem}, or <code>null</code> when request processing can be
     *         stopped
     */
    ...
    /**
     * Determine if the current request is an cross-context include, as typically exercised by Portals dispatching to a
     * targetted portlet
     *
     * @param request
     * @return
    ...
    /**
     * Writes the specified message to a filter log file, prepended by the filter's name.  See {@link
    ...
    /**
     * Retrieves the init parameter from the filterConfig or servletContext. If the init parameter is not found in
     * filterConfig, then it will look up the init parameter from the servletContext. If either filterConfig or
     * servletContext is null, then either is not used to look up the init parameter. If the parameter is not found,
     * then it will return the defaultValue.
     *
