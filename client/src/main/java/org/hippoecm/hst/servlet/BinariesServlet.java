...
public class BinariesServlet extends HttpServlet {
    ...
    private void copyPageToResponse(HttpServletRequest request, HttpServletResponse response,
        ...
        try {
            ...
            if (offset >= 0 || length >= 0) {
                copyLarge(input, output, offset, length);
            } else {
    ...
    /*
     * NOTE: This method is copied from commons-io:2.x since commons-io:1.x (which HST-4.x depends on) doesn't
     *       have this.
     *       In HST-5.x or later versions, simply use IOUtils.copyLarge(...) methods instead.
     *       This method is not needed since HST-5.0.
     */
    private static long copyLarge(final InputStream input, final OutputStream output, final long inputOffset,
     ...
     * NOTE: This method is copied from commons-io:2.x since commons-io:1.x (which HST-4.x depends on) doesn't
     *       have this.
     *       In HST-5.x or later versions, simply use IOUtils.skipFully(...) methods instead.
     *       This method is not needed since HST-5.0.
     */
    private static void skipFully(final InputStream input, final long toSkip) throws IOException {
        if (toSkip < 0) {
     ...
     * NOTE: This method is copied from commons-io:2.x since commons-io:1.x (which HST-4.x depends on) doesn't
     *       have this.
     *       In HST-5.x or later versions, simply use IOUtils.skip(...) methods instead.
     *       This method is not needed since HST-5.0.
     */
    private static long skip(final InputStream input, final long toSkip) throws IOException {
        ...
        long remain = toSkip;
        while (remain > 0) {
            final long n = input.read(buffer, 0, (int) Math.min(remain, 2048));
            if (n < 0) { // EOF
                break;
            }
            remain -= n;
